                        ...
                        <table class="table align-middle table-row-bordered mb-0 fs-6 gy-5 min-w-300px">
                            <tbody class="fw-bold text-gray-600">
                                ...
                                <tr>
                                    <td class="text-muted">
                                        <div class="d-flex align-items-center">
                                            <span><i class="fa fa-tv mx-2"></i></span> {{ __('Live Event Title') }}
                                        </div>
                                    </td>
                                    <td class="fw-bolder text-end">
                                        @if ($order->live)
                                            {{ $order->live->title_ar }} / {{ $order->live->title_en }}
                                        @else
                                            {{ __('No live event associated') }}
                                        @endif
                                    </td>
                                </tr>
                                ...
                                <tr>
                                    <td class="text-muted">
                                        <div class="d-flex align-items-center">
                                            <span><i class="fa fa-calendar mx-2"></i></span> {{ __('Live Date') }}
                                        </div>
                                    </td>
                                    <td class="fw-bolder text-end">
                                        @if ($order->live)
                                            {{ \Carbon\Carbon::parse($order->live->day_date)->format('Y-m-d') }}
                                        @else
                                            {{ __('N/A') }}
                                        @endif
                                    </td>
                                ...
                                <tr>
                                    <td class="text-muted">
                                        <div class="d-flex align-items-center">
                                            <span><i class="fa fa-clock mx-2"></i></span> {{ __('Live Time') }}
                                        </div>
                                    </td>
                                    <td class="fw-bolder text-end">
                                        @if ($order->live)
                                            {{ \Carbon\Carbon::parse($order->live->from)->format('H:i A') }} -
                                            {{ \Carbon\Carbon::parse($order->live->to)->format('H:i A') }}
                                        @else
                                            {{ __('N/A') }}
                                        @endif
                                    </td>
                                ...
                                <tr>
                                    ...
                                    <td class="fw-bolder text-end">{{ $order->vendor->name ?? 'N/A' }}</td>
                                </tr>
